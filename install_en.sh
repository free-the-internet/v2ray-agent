...
#Install toolkit
installTools() {
	echoContent skyBlue " \   nProgress $1 / ${totalProgress} : Install tool "
...
installWarp() {
	...
	#	if [[]];then
	#	fi
	# all curl --socks5 127.0.0.1:31303 https://www.cloudflare.com/cdn-cgi/trace
	# systemctl daemon-reload
...
initTLSNginxConfig() {
	...
	if [[ -n "${currentHost}" ]]; then
		...
		echo
		read -r -p " Please pay attention to the correctness of the detection, do you use the domain name of the last installation? [y/n]: " historyDomainStatus
...
checkIP() {
	...
	if [[ -z ${localIP} ]] || ! echo "${localIP}" | sed '1{s/[^(]*(//;s/).*//;q}' | grep -q '\.' && ! echo "${localIP}" | sed '1{s/[^(]*(//;s/).*//;q}' | grep -q ':'; then
		...
		if [[ -n ${localIP} ]]; then
			echoContent yellow " ---> The detection return value is abnormal. It is recommended to re-execute the script after manually uninstalling nginx "
		fi
		...
		read -r -p " Do you want to modify the firewall rules to open ${portFirewallPortStatus} port through script? [y/n]: " allPortFirewallStatus
...
customPortFunction() {
	...
	if [[ -n "${customPort}" || -n "${currentPort}" ]]; then
		echo
		read -r -p " Read the port of the last installation, whether to use the port of the last installation? [y/n]: " historyCustomPortStatus
...
installTLS() {
	...
	if [[ -f "/etc/v2ray-agent/tls/${tlsDomain}.crt" && -f "/etc/v2ray-agent/tls/${tlsDomain}.key" && -n $(cat "/etc/v2ray-agent/tls/${tlsDomain}.crt") ]] || [[ -d "$HOME/.acme.sh/${tlsDomain}_ecc" && -f "$HOME/.acme.sh/${tlsDomain}_ecc/${tlsDomain}.key" && -f "$HOME/.acme.sh/${tlsDomain}_ecc/${tlsDomain}.cer" ]]; then
		...
		if [[ -z $(find /etc/v2ray-agent/tls/ -name "${tlsDomain}.crt") ]] || [[ -z $(find /etc/v2ray-agent/tls/ -name "${tlsDomain}.key") ]] || [[ -z $(cat "/etc/v2ray-agent/tls/${tlsDomain}.crt") ]]; then
			...
			echoContent yellow " ---> Please select [n]\n " if the certificate has not expired or is a custom certificate
		...
		if [[ "${installDNSACMEStatus}" != "true" ]]; then
			...
			echoContent green " ---> Detected that a wildcard certificate has been installed and is being automatically generated "
		fi
		if [[ "${installDNSACMEStatus}" == "true" ]]; then
			echo
			if [[ -d "$HOME/.acme.sh/*.${dnsTLSDomain}_ecc" && -f "$HOME/.acme.sh/*.${dnsTLSDomain}_ecc/*.${dnsTLSDomain}.key" && -f "$HOME/.acme.sh/*.${dnsTLSDomain}_ecc/*.${dnsTLSDomain}.cer" ]]; then
...
installCronTLS() {
	...
	echoContent green " \n ---> Added scheduled maintenance certificate successfully "
}
...
installHysteria() {
	readInstallType
	echoContent skyBlue " \   nProgress $1 / ${totalProgress} : Installing Hysteria "
...
v2rayVersionManageMenu() {
	...
	if [[ "${selectV2RayType}" == "1" ]]; then
		...
		echoContent yellow " 3. If the rolled back version does not support the current config, you will not be able to connect, please proceed with caution "
...
installV2RayService() {
	echoContent skyBlue " \   nProgress $1 / ${totalProgress} : Configure V2Ray to start automatically "
...
installHysteriaService() {
	echoContent skyBlue " \   nProgress $1 / ${totalProgress} : Configure Hysteria to start automatically "
	if [[ -n $(find /bin /usr/bin -name "systemctl") ]]; then
...
installXrayService() {
	echoContent skyBlue " \   nProgress $1 / ${totalProgress} : Configure Xray to start automatically "
	if [[ -n $(find /bin /usr/bin -name "systemctl") ]]; then
...
EOF
		...
		echoContent green " ---> Configure Xray to start automatically after booting successfully "
	fi
}
...
handleHysteria() {
	...
	if [[ "$1" == "start" ]]; then
		if [[ -n $(pgrep -f "hysteria/hysteria") ]]; then
			...
			echoContent red " Please manually execute [/etc/v2ray-agent/hysteria/hysteria --log-level debug -c /etc/v2ray-agent/hysteria/conf/config.json server] to check the error log "
			exit 0
		fi
	elif [[ "$1" == "stop" ]]; then
...
initHysteriaPort() {
	...
	if [[ -n "${hysteriaPort}" ]]; then
		read -r -p " Read the port of the last installation, whether to use the port of the last installation? [y/n]: " historyHysteriaPortStatus
	...
	if [[ -z ${hysteriaPort} ]]; then
		...
		echoContent red " ---> port is invalid "
		initHysteriaPort "$2"
	fi
...
initHysteriaConfig() {
	echoContent skyBlue " \ nProgress $1 / ${totalProgress} : initializing Hysteria configuration "
...
initXrayFrontingConfig() {
	...
	echoContent yellow " If the front is Trojan, there will be two Trojan protocol nodes when viewing the account, and one of them is unavailable xtls "
...
initTrojanGoConfig() {
	...
	echoContent skyBlue " \ nProgress $1 / ${totalProgress} : Initializing Trojan configuration "
...
#Custom CDN IP
customCDNIP() {
	echoContent skyBlue " \ nProgress $1 / ${totalProgress} : add cloudflare optional CNAME "
	echoContent red "\n=============================================================="
...
EOF
		...
		echoContent green "     Protocol type: VLESS, address: ${currentAdd} , masquerade domain name/SNI: ${currentHost} , port: ${currentDefaultPort} , user ID: ${id} , security: tls, transmission method: ws, path :/ ${currentPath} ws, account name: ${email} -vless-ws\n "
...
EOF
		...
		echoContent green "     Protocol type: VLESS, address: ${currentAdd} , masquerading domain name/SNI: ${currentHost} , port: ${currentDefaultPort} , user ID: ${id} , security: tls, transmission method: gRPC, alpn :h2, serviceName: ${currentPath} grpc, account name: ${email} -vless-grpc\n "
...
updateNginxBlog() {
	echoContent skyBlue " \ nProgress $1 / ${totalProgress} : Replace the masquerade site "
	echoContent red "=============================================================="
	...
	echoContent yellow " 10.302 redirected website "
	echoContent red "=============================================================="
	...
	if [[ "${selectInstallNginxBlogType}" == "10" ]]; then
		...
		echoContent yellow " If you want to fake the site to achieve the effect, you need to delete the 302 redirection configuration\n "
	...
	if [[ "${selectInstallNginxBlogType}" =~ ^[1-9]$ ]]; then
		...
		echoContent green " ---> Replaced fake site successfully "
	else
		echoContent red " ---> selection error, please re-select "
		updateNginxBlog
	fi
}
...
unInstall() {
	...
	if [[ "${unInstallStatus}" != "y" ]]; then
		echoContent green " ---> give up unloading "
		menu
		exit 0
	fi
	...
	if [[ "${coreInstallType}" == "1" ]]; then
		handleXray stop
		rm -rf /etc/systemd/system/xray.service
		echoContent green " ---> delete Xray boot self-start complete "
	...
	if [[ -z "${hysteriaConfigPath}" ]]; then
		handleHysteria stop
		rm -rf /etc/systemd/system/hysteria.service
		echoContent green " ---> Delete Hysteria boot self-start completed "
	fi
...
bbrInstall () {
	echoContent red "\n=============================================================="
	echoContent green " The mature work of [ylx2016] for BBR and DD scripts, the address [https://github.com/ylx2016/Linux-NetSpeed], please familiarize yourself with it "
	echoContent yellow " 1. Installation script【recommended original BBR+FQ】"
	echoContent yellow " 2. Fall back to the main directory "
	echoContent red "=============================================================="
...
checkLog() {
	...
	echoContent red "\n=============================================================="
	echoContent yellow " # It is recommended to open the access log only for debugging\n "
...
blacklist() {
	...
	echoContent skyBlue " \   nProgress $1 / ${totalProgress} : domain name blacklist "
	echoContent red "\n=============================================================="
	echoContent yellow " 1. Add domain name "
...
# warp split
warpRouting() {
	echoContent skyBlue " \   nProgress $1 / ${totalProgress} : WARP split "
	echoContent red "=============================================================="
	...
	echoContent red "\n=============================================================="
	echoContent yellow " 1. Add domain name "
	echoContent yellow " 2. Unload WARP distribution "
	echoContent red "=============================================================="
...
dokodemoDoorUnblockStreamingMedia() {
	...
	echoContent red "\n=============================================================="
	echoContent yellow " # Notes "
	echoContent yellow " For details on unlocking any door, please check this article [https://github.com/reeceyng/v2ray-agent/blob/master/documents/netflix/dokodemo-unblock_netflix.md]\n "
...
customV2RayInstall() {
	...
	if [[ "${selectCustomInstallType}" =~ ^[0-5]+$ ]]; then
		...
		echoContent red " ---> input is invalid "
		customV2RayInstall
	fi
}
...
selectCoreInstall() {
	echoContent skyBlue " \nfeature 1/ ${totalProgress} : select core install "
	echoContent red "\n=============================================================="
...
switchAlpn() {
	...
	if [[ -z ${currentAlpn} ]]; then
		echoContent red " ---> cannot read alpn, please check if it is installed "
		exit 0
	fi

	echoContent red "\n=============================================================="
	echoContent green "The current alpn first is: ${currentAlpn} "
	echoContent yellow "   1. When http/1.1 is the first, trojan is available, and some gRPC clients are available [the client supports manual selection of alpn available] "
...
menu() {
	...
	echoContent yellow " 11. Streaming media tool "
